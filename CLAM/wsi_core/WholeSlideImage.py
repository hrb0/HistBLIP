...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            asset_dict, attr_dict = self.process_cus(cont, self.holes_tissue[idx], patch_level, save_path, patch_size, step_size, **kwargs)
            if len(asset_dict) > 0:
                if init:
    ...
    def process_cus(self, cont, contour_holes, patch_level, save_path, patch_size=256, step_size=256,
                contour_fn='four_pt', use_padding=True, top_left=None, bot_right=None):
        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.level_dim[patch_level][0], self.level_dim[patch_level][1])
        ...
        results = pool.starmap(WholeSlideImage.process_patch_candidate, [(coord, patch_level, patch_size, ref_patch_size, slide_path) for coord in coord_candidates])

        pool.close()
        pool.join()

        results = [result for result in results if result is not None]
        print(f'Extracted {len(results)} valid patches')

        if len(results) > 0:
            asset_dict = {'coords': np.array([[r[0], r[1]] for r in results])}
            attr = {
    ...
    @staticmethod
    def process_patch_candidate(coord, patch_level, patch_size, ref_patch_size, slide_path):
        x, y = coord

        slide = openslide.open_slide(slide_path)

        image_patch = np.array(slide.read_region(location=(x, y), level=patch_level, size=(ref_patch_size[0], ref_patch_size[1]))).astype(np.uint8)[..., :3]
        
        resize_image = cv2.resize(image_patch, (patch_size, patch_size), cv2.INTER_AREA)

        if (np.mean(resize_image) < 255 * 0.9) and (np.mean(resize_image) > 255 * 0.3):
            return (x, y, resize_image)
        else:
            return None
