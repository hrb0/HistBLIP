...
class AttModel(CaptionModel):
    ...
    def _sample_beam_BLIP(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        # first step, feed bos
        it = fc_feats.new_full([batch_size], self.bos_idx, dtype=torch.long)
        logprobs, state = self.get_logprobs_state_BLIP(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
    ...
    def _sample_BLIP(self, fc_feats, att_feats, att_masks=None, update_opts={}):
        ...
        for t in range(self.max_seq_length + 1):
            ...
            logprobs, state = self.get_logprobs_state_BILP(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state,
                                                      output_logsoftmax=output_logsoftmax)
