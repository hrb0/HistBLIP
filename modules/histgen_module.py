    ...
    def forward(self, x, memory, src_mask, tgt_mask, past=None):
        ...
        for i, (layer, layer_past) in enumerate(zip(self.layers, past)):
            ...
            if layer_past is not None:
                ...
                x = x[0]
        if past[0] is None:
            # print('past[0] is None')
            # print(xxx)
            return self.norm(x)
        else:
            # print('keys: ',torch.cat(present[0], 0).shape)
            # print('values: ',torch.cat(present[1], 0).shape)
            # print('len(keys[present[0]]): ',len(present[0]))
            # print('keys_layers[0][0]: ', present[0][0].shape)
            # print(xxx)
...
class BaseHistGen(AttModel):
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        ...
        # print('past_before1: ',past[0][0].shape)
        # print('past_before2: ',past[0][1].shape)
        ...
        # print('past_after1: ',past[0][0].shape)
        # print('past_after2: ',past[0][1].shape)
        if not self.training:
            self._save_attns(start=len(state) == 0)
        #print('ys: ',ys.shape)
        
        # print('out_hist: ',ys.unsqueeze(0).shape)
        # print(xxx)
        # print('out_state: ',([ys.unsqueeze(0)] + past)[0][0][:0].shape)
        #print('out_state: ',past[0][0].shape)
