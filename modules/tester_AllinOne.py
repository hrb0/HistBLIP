...
class Tester_BLIP(BaseTester):
    def __init__(self, model, criterion, metric_ftns, args, test_dataloader):
        super(Tester_BLIP, self).__init__(model, criterion, metric_ftns, args)
        self.test_dataloader = test_dataloader
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks) in tqdm(enumerate(self.test_dataloader)):
                images_id, images, reports_ids, reports_masks = images_id[0], images.to(self.device), reports_ids.to(
                    self.device), reports_masks.to(self.device)
                output = self.model(images, mode='sample')
                reports = [self.model.tokenizer.decode(ids, clean_up_tokenization_spaces=True,skip_special_tokens=True) for ids in output]
                ground_truths = [self.model.tokenizer.decode(ids, clean_up_tokenization_spaces=True,skip_special_tokens=True) for ids in reports_ids]
                if self.args.format != 'full':
                    for num_batch in range(len(ground_truths)):
                        reports[num_batch] = 'urinary bladder, transurethral resection;/' + reports[num_batch]
                        ground_truths[num_batch] = 'urinary bladder, transurethral resection;/' + ground_truths[num_batch]
                test_res.extend(reports)
                test_gts.extend(ground_truths)
                test_ids.append(images_id)
