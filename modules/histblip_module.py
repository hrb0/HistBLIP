...
class Transformer(nn.Module):
    def __init__(self, encoder, src_embed, cmn, d_model, num_heads,bos_idx,eos_idx,pad_idx,max_length,num_beams,topk,temperature,tokenizer):
        ...
        # Load BLIP model and extract the text decoder
        self.blip_model = BlipForConditionalGeneration.from_pretrained("Salesforce/blip-image-captioning-base")
        self.text_decoder = self.blip_model.text_decoder
    ...
    def forward(self, src, tgt_input_ids, src_mask, tgt_mask, memory_matrix):
        memory, src_mask  = self.encode(src, src_mask)
        outputs = self.decode(memory, src_mask, tgt_input_ids, tgt_mask, memory_matrix=memory_matrix)
        return outputs

    def encode(self, src, src_mask):
        # Pass through your custom HATEncoder
        encoder_outputs = self.encoder(self.src_embed(src), src_mask)
        src_mask = torch.ones(encoder_outputs.size()[:2], dtype=torch.long, device=encoder_outputs.device)
        return encoder_outputs, src_mask   # Shape: [batch_size, seq_len, hidden_size]

    def decode(self, memory, src_mask, tgt_input_ids, tgt_mask, memory_matrix=None,mode='train'):
        if mode == 'train': 
...
class BaseHistBLIP(AttModel):
    def make_model(self, cmn, encoder_layout=None):
        ...
        model = Transformer(
            ...
            cmn,
            self.d_model,
            self.num_heads,self.bos_idx,self.eos_idx,self.pad_idx,self.max_length,self.beam_size,self.topk,self.temperture,self.tokenizer
        )
        return model

    def __init__(self, args, tokenizer):
        super(BaseHistBLIP, self).__init__(args, tokenizer)
        self.args = args
        ...
        self.temperture = args.temperature
        self.tokenizer = tokenizer
        ...
        self.model = self.make_model(self.cmn, self.encoder_layout)
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        ...
        memory, att_masks = self.model.encode(att_feats, att_masks)
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        outputs = self.model(att_feats, seq, att_masks, seq_mask, memory_matrix=self.memory_matrix)
        #outputs = F.log_softmax(self.logit(out), dim=-1)

        return outputs

    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        memory = memory[0] if isinstance(memory, tuple) else memory
        ...
        out = self.model.decode(memory, mask, ys, subsequent_mask(ys.size(1)).to(memory.device),
                                      memory_matrix=self.memory_matrix,mode='sample')
