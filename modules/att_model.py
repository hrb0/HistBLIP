...
class AttModel(CaptionModel):
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1):
        ...
        #print('output: ',self.logit(output).shape)
    ...
    def get_logprobs_state_BLIP(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1):
        # 'it' contains a word index
        xt = self.embed(it)

        output = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks)
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        #print(self.done_beams)
        #print('self.done_beams.shape: ',len(self.done_beams))
    ...
    def _sample_beam_BLIP(self, fc_feats, att_feats, att_masks=None, opt={}):
        #print('_sample_beam')
        beam_size = opt.get('beam_size', 10)
        group_size = opt.get('group_size', 1)
        sample_n = opt.get('sample_n', 10)
        ...
        # first step, feed bos
        it = fc_feats.new_full([batch_size], self.bos_idx, dtype=torch.long)
        seq = self.get_logprobs_state_BLIP(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
        
        return seq
    
    def _sample_BLIP(self, fc_feats, att_feats, att_masks=None, update_opts={}):
        opt = self.args.__dict__
        opt.update(**update_opts)
        ...
        beam_size = opt.get('beam_size', 1)
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam_BLIP(fc_feats, att_feats, att_masks, opt)
